...
class TestUser:
    def test_user(self, connect, dataset):  # pylint: disable=unused-argument
        name = random_string()
        user = User(
            name,
            random_string(),
    ...
    def test_users(self, connect, dataset):  # pylint: disable=unused-argument
...
class TestCurrentUser:
    def test_current_user(self, connect, dataset):  # pylint: disable=unused-argument
...
class TestUserJob:
    def test_user_job(self, connect, dataset):  # pylint: disable=unused-argument
        pass
    ...
    def test_user_jobs_by_project(
        self,
        connect,  # pylint: disable=unused-argument
        dataset,
    ):
        user_jobs = UserJobCollection().fetch_with_filter(
            "project",
            dataset["project"].id,
        )
        assert isinstance(user_jobs, UserJobCollection)
...
class TestGroup:
    def test_group(self, connect, dataset):  # pylint: disable=unused-argument
        name = random_string()
    ...
    def test_groups(self, connect, dataset):  # pylint: disable=unused-argument
...
class TestUserGroup:
    def test_user_group(self, connect, dataset):  # pylint: disable=unused-argument
    ...
    def test_user_groups_by_user(
        self,
        connect,  # pylint: disable=unused-argument
        dataset,
    ):
        user_groups = UserGroupCollection().fetch_with_filter(
            "user",
            dataset["user"].id,
        )
        assert isinstance(user_groups, UserGroupCollection)
...
class TestRole:
    def test_roles(self, connect, dataset):  # pylint: disable=unused-argument
        roles = RoleCollection().fetch()
        assert isinstance(roles, RoleCollection)

    def test_role(self, connect, dataset):  # pylint: disable=unused-argument
        roles = RoleCollection().fetch()
        if len(roles) > 0:
...
class TestUserRole:
    def test_user_role(self, connect, dataset):  # pylint: disable=unused-argument
        roles = RoleCollection().fetch()
        if len(roles) > 0:
