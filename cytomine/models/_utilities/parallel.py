...
def generic_chunk_parallel(data, worker_fn, chunk_size=1, n_workers=0):
    ...
    -------
    results: iterable
        List processed items as tuples.
        First element of the tuple is the slice (start,end) of the chunk (end excluded),
...
def generic_download(data, download_instance_fn, n_workers=0):
    ...
    results: iterable
        List processed items as tuples. First element of the tuple is the item itself,
        the second element of the tuple is the value returned by `download_instance_fn`
        for this item.
    """
