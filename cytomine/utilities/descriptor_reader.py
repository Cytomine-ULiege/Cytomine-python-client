...
def _format_type(type):  # pylint: disable=redefined-builtin
...
def read_descriptor(  # pylint: disable=too-many-branches
    filename,
    ...
    The software can be launched from the command line by the developer,
    faking a real execution from Cytomine WebUI.
    If the software already exists, it is updated.

    DO NOT USE this to provide a production-ready software, it won't work.
    In particular, image pulling is by-passed, software and parameter descriptions
    are not taken into account, nor parameter constraints.
    ...
    with open(filename, "r", encoding="utf-8") as f:
        ...
        existing_software = [
            s
            for s in existing_softwares
            if s.name == descriptor["name"] and not s.softwareVersion
        ]
        ...
        existing_software_parameters = SoftwareParameterCollection().fetch_with_filter(
            "software",
            software.id,
        )
        ...
        for parameter_descriptor in descriptor["inputs"]:
            ...
            param = {
                k: (
                    v.replace("@ID", parameter_descriptor["id"].upper()).replace(
                        "@id", parameter_descriptor["id"].lower()
                    )
                    if isinstance(v, str)
                    else v
                )
            ...
            sp = SoftwareParameter(
                ...
                default_value=(
                    param["default-value"] if "default-value" in param.keys() else ""
                ),
                required=(
                    not param["optional"] if "optional" in param.keys() else False
                ),
                set_by_server=(
                    param["set-by-server"] if "set-by-server" in param.keys() else False
                ),
                uri=(param["uri"] if "uri" in param.keys() else None),
                uri_sort_attribut=(
                    param["uri-print-attribute"]
                    if "uri-print-attribute" in param.keys()
                    else None
                ),
                uri_print_attribut=(
                    param["uri-print-attribute"]
                ...
                server_parameter=(
                    ...
                    if "server-parameter" in param.keys()
                    else False
                ),
            ...
            existing_software_parameter = [
                p for p in existing_software_parameters if p.name == param["id"]
            ]
