...
def _cytomine_parameter_name_synonyms(name, prefix="--"):
    ...
    -------
    names: str
        List of prefixed parameter names containing at least $name
        (preprended with $prefix).
    """
    synonyms = [
        [
            "host",
...
class Cytomine:
    ...
    def connect_from_cli(cls, argv, use_cache=True):
        ...
        Notes
        -----
        If some parameters are invalid, the function
        stops the execution and displays an help.
        """
    ...
    def wait_to_accept_connection(
        ...
        delay_between_retry_in_seconds=1,
    ):
